  ...
  while (true) {
	...
	break;
      ...
      dout(20) << __func__ << " committing " << kv_queue.size()
	       ...
	       << dendl;
      auto batch_process_start = mono_clock::now();
...
void BlueStore::BlueStoreCoDel::register_batch(int64_t queuing_latency, int64_t batch_size, int64_t txc_num) {
    if(activated){
...
void BlueStore::BlueStoreCoDel::init(CephContext* cct) {
    ...
    if (cct->_conf.get_val<int64_t>("bluestore_codel_init_batch_size")) {
    ...
    if (getline(settingFile, line)) {
        ...
        initial_batch_size = initial_batch_size * 1024;
    }
    settingFile.close();
