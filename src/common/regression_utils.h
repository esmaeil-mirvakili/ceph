...
#ifndef CEPH_REGRESSION_UTILS_H
#define CEPH_REGRESSION_UTILS_H
...
namespace ceph {
  ...
  */
  ...
  /***
  * Find a logarithmic function in form of "y = a + b * ln(x)" which fits the given points (x_values and y_values).
  ...
  */
  static void logarithmic_regression(
  ...
  /***
  * Finds the x location on a fitted logarithmic curve on sample points where the slope is equal to target_slope
  ...
  */
  static double find_slope_on_logarithmic_curve(
    const std::vector<double> &x_values,
    const std::vector<double> &y_values,
    double target_slope) {
    assert(x_values.size() == y_values.size() && "x and y values vectors should have a same size.");
  ...
  /***
  * Finds the mu and std parameters of the lognormal distribution from its mode and x boundaries.
