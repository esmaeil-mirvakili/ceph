...
void CoDel::_coarse_interval_process() {
    ...
    if (!mono_clock::is_zero(slow_interval_start) && txc_cnt > 0) {
        ...
        auto cur_loss = (500.0 * pow(0.9, avg_lat*3)) + (0.5*pow(0.9, -cur_throughput/2));
        auto pre_loss = (500.0 * pow(0.9, slow_interval_lat*3)) + (0.5*pow(0.9, -slow_interval_throughput/2));
